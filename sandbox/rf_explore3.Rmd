...
The markdown is used for covariate exploration for random forest models. It takes you throught several sections including: 
  ...
  Section 5: RF Best Fit - Test the performance of the best fit model 
  Section 6: Additional RF Tuning - Explore different tuning options to improve model fit 
...
# Load data 
dat <- read_csv("data/outbreaks_covariates_dataset.csv")
...
Fit the random forest model using the caret package 
...
# Fit the model 
rf <- train(attack_rate~., data=train, method='rf', 
            metric='RMSE', tuneGrid=tunegrid, trControl=control)

```

## Section 5: RF Variable Importance  ------------------------------------------------ 
...
## Section 6: RF Performance  ------------------------------------------------

Test the performance of the model using the actual vs predicted attack rates 

```{r rf perform}
# Make predictions on the testing set 
p <- predict(rf, test)
# Extract the performance metrics 
postResample(p, test$attack_rate)
cor(test$attack_rate, p)
```
